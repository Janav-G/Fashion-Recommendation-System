...
if uploaded_file is not None:
 ...
 # show
        ...
        with col1:
            ...
            st.image(recommended_image, caption=filenames[indices[0][0]])
            st.markdown(f"<style>div.stImage > img {{ margin: {margin_width}px; }}</style>", unsafe_allow_html=True)

        with col2:
            recommended_image = Image.open(filenames[indices[0][1]])
            st.image(recommended_image, caption=filenames[indices[0][1]])
            st.markdown(f"<style>div.stImage > img {{ margin: {margin_width}px; }}</style>", unsafe_allow_html=True)
        with col3:
            recommended_image = Image.open(filenames[indices[0][2]])
            st.image(recommended_image, caption=filenames[indices[0][2]])
            st.markdown(f"<style>div.stImage > img {{ margin: {margin_width}px; }}</style>", unsafe_allow_html=True)
        with col4:
            recommended_image = Image.open(filenames[indices[0][3]])
            st.image(recommended_image, caption=filenames[indices[0][3]])
            st.markdown(f"<style>div.stImage > img {{ margin: {margin_width}px; }}</style>", unsafe_allow_html=True)
        with col5:
            recommended_image = Image.open(filenames[indices[0][4]])
            st.image(recommended_image, caption=filenames[indices[0][4]])
            st.markdown(f"<style>div.stImage > img {{ margin: {margin_width}px; }}</style>", unsafe_allow_html=True)"""